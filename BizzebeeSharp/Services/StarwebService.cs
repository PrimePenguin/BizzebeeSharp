...
using BizzebeeSharp.Infrastructure.Policies;

namespace BizzebeeSharp.Services
{
    public abstract class BizzebeeService
    {
        ...
        /// <summary>
        /// Creates a new instance of <see cref="BizzebeeService" />.
        /// </summary>
        /// <param name="myBizzebeeUrl">The shop's *.myBizzebee.com URL.</param>
        /// <param name="shopAccessToken">An API access token for the shop.</param>
        protected BizzebeeService(string myBizzebeeUrl, string shopAccessToken)
        {
            _ShopUri = BuildShopUri(myBizzebeeUrl);
        ...
        /// <summary>
        /// Attempts to build a shop API <see cref="Uri"/> for the given shop. Will throw a <see cref="BizzebeeException"/> if the URL cannot be formatted.
        /// </summary>
        /// <param name="myBizzebeeUrl">The shop's *.Bizzebeeserver.se URL.</param>
        /// <exception cref="BizzebeeException">Thrown if the given URL cannot be converted into a well-formed URI.</exception>
        /// <returns>The shop's API <see cref="Uri"/>.</returns>
        public static Uri BuildShopUri(string myBizzebeeUrl)
        {
            if (Uri.IsWellFormedUriString(myBizzebeeUrl, UriKind.Absolute) == false)
            {
                //Bizzebee typically returns the shop URL without a scheme. If the user is storing that as-is, the uri will not be well formed.
                //Try to fix that by adding a scheme and checking again.
                if (Uri.IsWellFormedUriString("https://" + myBizzebeeUrl, UriKind.Absolute) == false)
                {
                    throw new BizzebeeException($"The given {nameof(myBizzebeeUrl)} cannot be converted into a well-formed URI.");
                }

                myBizzebeeUrl = "https://" + myBizzebeeUrl;
            }

            var builder = new UriBuilder(myBizzebeeUrl)
            {
                Scheme = "https:",
                Port = 443 //SSL port
            };
        ...
        public static void CheckResponseExceptions(HttpResponseMessage response, string rawResponse)
        {
            ...
            //    // Bizzebee used to return JSON for rate limit exceptions, but then made an unannounced change and started returing HTML.
            ...
            //    throw new BizzebeeRateLimitException(code, rateLimitErrors, rateLimitMessage, rawResponse, requestId);
            //}
            ...
            if (error == null)
            {
                throw new BizzebeeException(code, $"{(int)code} {response.ReasonPhrase}", message);
            }

            throw new BizzebeeException(code, error.Error, error.ErrorDescription);
        }

        /// <summary>
        /// Parses a JSON string for Bizzebee API errors.
        /// </summary>
